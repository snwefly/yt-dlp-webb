...
def register_routes(app):
    ...
    def get_video_info():
        ...
        try:
            ...
            return jsonify({'error': f'无法获取视频信息: {error_message}'}), 500
    ...
    def shortcuts_download_direct():
        ...
        try:
            ...
            return jsonify({'success': False, 'error': f'下载失败: {error_message}'}), 500
    ...
    def get_available_formats():
        ...
        try:
            ...
            err_msg = str(e.args[0]) if hasattr(e, 'args') and e.args else str(e)
            return jsonify({'error': f'获取格式失败: {err_msg}'}), 500
    ...
    def start_update():
        try:
            ...
            return jsonify({'error': f'启动更新失败: {str(e)}'}), 500
    ...
    return app
# (接在 create_app 函数的 "return app" 之后)
...
def register_routes(app):
    ...
    @app.route('/login', methods=['GET', 'POST'])
    def login():
        if get_current_user(): # 如果已登录，直接重定向
            return redirect(url_for('index'))
        if request.method == 'POST':
            username = request.form.get('username')
            password = request.form.get('password')
    ...
    @login_required
    def logout_route():
        user = get_current_user() # 获取当前用户名用于日志
        auth_token = session.pop('auth_token', None)
        session.pop('username', None) # 确保username也清除
        if auth_token:
            auth_manager.destroy_session(auth_token)
        logger.info(f"用户 {user or '(unknown)'} 已登出。")
        return redirect(url_for('login'))
    ...
    def api_login():
        try:
            ...
            if auth_manager.verify_credentials(username, password):
                session_token = auth_manager.create_session(username)
                # API 登录也设置 Flask session，以便后续请求可以通过 cookie 认证
                session['auth_token'] = session_token 
                session['username'] = username
                logger.info(f"用户 {username} 通过 API 登录成功。")
                return jsonify({'success': True, 'message': '登录成功', 'token': session_token, 'username': username})
    ...
    def api_logout():
        try:
            token_to_destroy = None
            user_logout_identifier = "User (token/session not found)" # 用于日志
            auth_header = request.headers.get('Authorization')
            if auth_header and auth_header.startswith('Bearer '):
                token_to_destroy = auth_header.split(' ')[1]
                user_logout_identifier = f"User (token: ...{token_to_destroy[-4:]})"
            elif 'auth_token' in session: # 其次处理 Flask session 中的 token
                token_to_destroy = session['auth_token']
                user_logout_identifier = f"User (session: {session.get('username', 'unknown')})"
            
            if token_to_destroy:
                retrieved_user = auth_manager.get_session_user(token_to_destroy)
                if retrieved_user: user_logout_identifier = f"User ({retrieved_user})"
                auth_manager.destroy_session(token_to_destroy)
            
            # 清理 Flask session
            session.pop('auth_token', None)
            session.pop('username', None)
            logger.info(f"{user_logout_identifier} 通过 API 登出。")
            return jsonify({'success': True, 'message': '已成功登出'})
        except Exception as e:
            logger.error(f"API登出时发生错误: {e}", exc_info=True)
            return jsonify({'error': '服务器内部错误'}), 500
    ...
    def api_verify():
        try:
            ...
            if auth_header and auth_header.startswith('Bearer '):
                token_to_verify = auth_header.split(' ')[1]
                is_from_header = True
            elif 'auth_token' in session: # 检查 Flask session 中的 token
                token_to_verify = session['auth_token']
            
            if token_to_verify and auth_manager.verify_session(token_to_verify):
                username = auth_manager.get_session_user(token_to_verify)
                if username: # 确保获取到用户名
                    # 如果通过 header 验证，并且 session 中没有，则同步 session
                    if is_from_header and (not session.get('auth_token') or session.get('username') != username):
                        session['auth_token'] = token_to_verify
                        session['username'] = username
                    return jsonify({'valid': True, 'username': username})
            
            # 如果 token 无效或不存在，清除 session
            session.pop('auth_token', None)
            session.pop('username', None)
            return jsonify({'valid': False, 'message': '会话无效或已过期'}), 401
        except Exception as e:
    ...
    def get_version_info():
        try:
            return jsonify(_make_json_serializable_recursive({
                'yt_dlp_version': _get_current_version(),
                'web_interface_version': os.environ.get('APP_VERSION', '1.2.3'), 
                'last_yt_dlp_update_check': _get_last_update_time() 
            }))
        except Exception as e:
            logger.error(f"获取版本信息时发生错误: {e}", exc_info=True)
...
def _generate_shortcut_config(shortcut_type):
    ...
    shortcuts_meta = {
        'smart_downloader': {
            "name": f"智能下载@{hostname}", 
            "description": "从剪贴板获取链接, 选择格式后通过yt-dlp-web下载。",
            "filename": "SmartVideoDownloader.shortcut", 
            "api_endpoint_async": f"{server_base_url}/api/shortcuts/download", 
            "api_endpoint_direct": f"{server_base_url}/api/shortcuts/download-direct",
            "status_url_pattern": f"{server_base_url}/api/download/{{DOWNLOAD_ID}}/status",
            "file_url_pattern": f"{server_base_url}/api/shortcuts/download/{{DOWNLOAD_ID}}/file"
        },
