...
def create_parser():
    ...
    general.add_option(
        '--web-server',
        action='store_true', dest='web_server', default=False,
        help='启动 yt-dlp 的网页服务器界面')
    general.add_option(
        '--web-host',
        dest='web_host', metavar='HOST', default='127.0.0.1',
        help='网页服务器绑定的主机地址 (默认: 127.0.0.1)')
    general.add_option(
        '--web-port',
        dest='web_port', metavar='PORT', type=int, default=8080,
        help='网页服务器绑定的端口 (默认: 8080)')
    general.add_option(
        '--web-download-folder',
        dest='web_download_folder', metavar='PATH', default='./downloads',
        help='网页界面下载文件保存的文件夹 (默认: ./downloads)')
    general.add_option(
        '--web-no-browser',
        action='store_true', dest='web_no_browser', default=False,
        help='启动网页服务器时不自动打开浏览器')
